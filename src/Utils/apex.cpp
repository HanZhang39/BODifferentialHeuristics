...
bool ApexPathPair::update_nodes_by_merge_if_bounded_g(const ApexPathPairPtr &other, const std::vector<double> eps, MergeStrategy s){
  // Returns true on sucessful merge and false if it failure
  if (this->id != other->id) {
    return false;
  }
  ...
  if (s == MergeStrategy::SMALLER_G2 || s == MergeStrategy::SMALLER_G2_FIRST){
    ...
    if (! is_bounded_g(new_apex, new_path_node, eps)){
      if (s == MergeStrategy::SMALLER_G2_FIRST && is_bounded_g(new_apex, other_path_node, eps)){
        new_path_node = other_path_node;
      }else{
        return false;
      }
    ...
    if (is_bounded_g(new_apex, this->path_node, eps)){
      new_path_node = this->path_node;
    }
    if (is_bounded_g(new_apex, other->path_node, eps)){
      if (new_path_node == nullptr){
        new_path_node = other->path_node;
      }else{
        if (rand() % 2 == 1){
